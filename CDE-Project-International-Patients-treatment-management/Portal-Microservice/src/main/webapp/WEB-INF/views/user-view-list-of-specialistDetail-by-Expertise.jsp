			...
			<div class="content list-container">
				...
				<div class="container">
					<form:form action="specialistsByExpertsise" method="GET"
						modelAttribute="AilmentCategory">
				...
				<div class="container result-container center border">
					<h3>Search Result</h3>
					<c:if test="${not empty specialistDetail }">
						<table class="table table-striped">
							...
							<tbody>
								<c:forEach items="${specialistDetail}" var="listValue"
									varStatus="loop">
									<tr>
										<td>${loop.index+1}</td>
										<td>${listValue.getName()}</td>
										<td>${listValue.getAreaOfExpertise()}</td>
										<td>${listValue.getExperienceInYears()}</td>
										<td>${listValue.getContactNumber()}</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
