...
public class IPTreatmentOfferingServiceImpl implements IPTreatmentOfferingService {
	...
	@Override
	public List<SpecialistDetail> findByAreaOfExpertise(AilmentCategory areaOfExpertise){
		return specialistRepository.findByAreaOfExpertise(areaOfExpertise);
	}
	
	@Override
    @Transactional
    public boolean updateTreatmentPackage(Integer packageId, @Valid PackageDetail packageDetail) {
        if(packageDetailRepository.existsById(packageId)) {
            //treatmentPackRepository.updateTreatmentPackages(packageId, ipTreatmentPackage.getTreatmentPackageId(),ipTreatmentPackage.getAilmentCategory());
            PackageDetail packageD= packageDetailRepository.findById(packageId).get();
            packageD.setCost(packageDetail.getCost());
            packageD.setTreatmentDuration(packageDetail.getTreatmentDuration());
	...
	@Override
    public SpecialistDetail addSpecialist(@Valid SpecialistDetail specialistDetail) {
        log.info("Start");
        SpecialistDetail sd=specialistRepository.save(specialistDetail);
        log.info("End");
        return sd;
