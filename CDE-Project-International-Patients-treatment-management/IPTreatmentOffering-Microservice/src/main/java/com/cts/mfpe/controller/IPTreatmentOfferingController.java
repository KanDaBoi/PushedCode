...
public class IPTreatmentOfferingController {
	...
	public ResponseEntity<String> addSpecialist(@Valid @RequestBody SpecialistDetail specialistDetail,
		...
		if (authorisingClient.authorizeTheRequest(requestTokenHeader)) {
			ipOfferingService.addSpecialist(specialistDetail);
	...
	public ResponseEntity<String> deleteSpecialist(@PathVariable Integer id,
			...
			throws AuthorizationException {
		if (authorisingClient.authorizeTheRequest(requestTokenHeader)) {
			boolean isRemoved = ipOfferingService.delete(id);
			if (!isRemoved) {
	...
	@ApiOperation(notes = "Returns the list of specialists based on their area of expertise", value = "Find specialists by AOE")
	public List<SpecialistDetail> getAllSpecialistByAreaOfExpertise(
			@ApiParam(name = "ailment", value = "Ailment that specialists can treat") @PathVariable("ailment") AilmentCategory ailment,
			@RequestHeader(value = "Authorization", required = true) String requestTokenHeader)
			throws AuthorizationException {
		if (authorisingClient.authorizeTheRequest(requestTokenHeader)) {
			return ipOfferingService.findByAreaOfExpertise(ailment);
		} else {
			throw new AuthorizationException("Not allowed");
		}
	}
	...
	@ApiOperation(notes = "Updates the IPTreatment packages for international patients", value = "Update treatment package for patients")
	public ResponseEntity<String> updatePackages(@PathVariable int packageId,
		...
		if (authorisingClient.authorizeTheRequest(requestTokenHeader)) {
			...
			boolean isUpdated = ipOfferingService.updateTreatmentPackage(packageId, packageDetail);
