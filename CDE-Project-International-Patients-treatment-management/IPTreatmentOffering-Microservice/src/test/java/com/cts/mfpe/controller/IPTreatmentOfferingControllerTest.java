...
class IPTreatmentOfferingControllerTest {
	...
	void testGetAllIPTreatmentPackageWithValidToken() throws Exception {
		...
		this.mockMvc.perform(get("/ipTreatmentPackages").header("Authorization", "@uthoriz@tionToken123"))
	...
	void testGetIPTreatmentPackageByNameWithValidToken() throws Exception {
		...
		this.mockMvc
				.perform(get("/ipTreatmentPackageByName/{ailment}/{packageName}", "ORTHOPAIDICS", "Package 1")
						.header("Authorization", "@uthoriz@tionToken123"))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$.treatmentPackageId", is(1))).andExpect(jsonPath("$.packageDetail.pid", is(101)));
	...
	void testGetInvalidIPTreatmentPackageByNameWithValidToken() throws Exception {
		...
		when(client.authorizeTheRequest("@uthoriz@tionToken123")).thenReturn(true);
		when(ipOfferingService.findIPTreatmentPackageByName(any(), anyString()))
				.thenThrow(new IPTreatmentPackageNotFoundException("IP Treatment Package not found"));

		this.mockMvc.perform(get("/ipTreatmentPackageByName/{ailment}/{packageName}", "ORTHOPAIDICS", "Package 4")
				.header("Authorization", "@uthoriz@tionToken123")).andExpect(status().isNotFound());
	}
	...
	void testGetIPTreatmentPackageByNameWithInvalidToken() throws Exception {
		...
		this.mockMvc
				.perform(get("/ipTreatmentPackageByName/ORTHOPAIDICS/Package 1").header("Authorization", "wrongtoken"))
				.andExpect(status().isForbidden());
	...
	@Test
	@DisplayName("Test getAllSpecialistByAreaOfExpertise() of the Controller without token")
	void testGetAllSpecialistByAreaOfExpertiseWithoutToken() throws Exception {
		this.mockMvc.perform(get("/specialistsByExpertsise/{ailment}",AilmentCategory.UROLOGY)).andExpect(status().isBadRequest());
	}

	@Test
	@DisplayName("Test getAllSpecialistByAreaOfExpertise() of the Controller with valid token")
	void testGetAllSpecialistByAreaOfExpertiseWithValidToken() throws Exception {
		when(client.authorizeTheRequest("@uthoriz@tionToken123")).thenReturn(true);
		when(ipOfferingService.findByAreaOfExpertise(AilmentCategory.UROLOGY)).thenReturn(Arrays.asList(specialist1));
		this.mockMvc.perform(
				get("/specialistsByExpertsise/{ailment}",AilmentCategory.UROLOGY)
						.header("Authorization", "@uthoriz@tionToken123"))
				.andExpect(status().isOk());
	}

	@Test
	@DisplayName("Test getAllSpecialistByAreaOfExpertise() of the Controller with invalid token")
	void testGetAllSpecialistByAreaOfExpertiseWithInvalidToken() throws Exception {
		when(client.authorizeTheRequest("wrongtoken")).thenReturn(false);
		this.mockMvc.perform(get("/specialistsByExpertsise/{ailment}",AilmentCategory.UROLOGY).header("Authorization", "wrongtoken"))
				.andExpect(status().isForbidden());
	}

	@Test
	@DisplayName("Test addSpecialist() of the Controller without token")
	void testAddSpecialistWithoutToken() throws Exception {
		SpecialistDetail specialist = new SpecialistDetail(203, "Dr Karman", AilmentCategory.UROLOGY, 9, 91323344556L);
		ObjectWriter ow = new ObjectMapper().writer().withDefaultPrettyPrinter();
		this.mockMvc.perform(post("/addSpecialist").contentType(MediaType.APPLICATION_JSON)
				.content(ow.writeValueAsString(specialist))).andExpect(status().isBadRequest());
	}

	@Test
	@DisplayName("Test addSpecialist() of the Controller with valid token")
	void testAddSpecialistWithValidToken() throws Exception {
		when(client.authorizeTheRequest("@uthoriz@tionToken123")).thenReturn(true);
		SpecialistDetail specialist = new SpecialistDetail(203, "Dr Karman", AilmentCategory.UROLOGY, 9, 91323344556L);
		ObjectWriter ow = new ObjectMapper().writer().withDefaultPrettyPrinter();
		this.mockMvc.perform(
				post("/addSpecialist").contentType(MediaType.APPLICATION_JSON)
				.content(ow.writeValueAsString(specialist))
						.header("Authorization", "@uthoriz@tionToken123"))
				.andExpect(status().isOk());
	}

	@Test
	@DisplayName("Test addSpecialist() of the Controller with invalid token")
	void testAddSpecialistWithInvalidToken() throws Exception {
		when(client.authorizeTheRequest("wrongtoken")).thenReturn(false);
		SpecialistDetail specialist = new SpecialistDetail(203, "Dr Karman", AilmentCategory.UROLOGY, 9, 91323344556L);
		ObjectWriter ow = new ObjectMapper().writer().withDefaultPrettyPrinter();
		
		this.mockMvc.perform(post("/addSpecialist").contentType(MediaType.APPLICATION_JSON)
				.content(ow.writeValueAsString(specialist)).header("Authorization", "wrongtoken"))
				.andExpect(status().isForbidden());
	}
	
	@Test
	@DisplayName("Test deleteSpecialist() of the Controller without token")
	void testDeleteSpecialistWithoutToken() throws Exception {
		this.mockMvc.perform(delete("/deleteSpecialist/{id}",201)).andExpect(status().isBadRequest());
	}	

	@Test
	@DisplayName("Test deleteSpecialist() of the Controller with valid token")
	void testDeleteSpecialistWithValidToken() throws Exception {
		when(client.authorizeTheRequest("@uthoriz@tionToken123")).thenReturn(true);
		when(ipOfferingService.delete(201)).thenReturn(true);
		this.mockMvc.perform(
				delete("/deleteSpecialist/{id}",201)
						.header("Authorization", "@uthoriz@tionToken123"))
				.andExpect(status().isOk());
	}

	@Test
	@DisplayName("Test deleteSpecialist() of the Controller with invalid token")
	void testDeleteSpecialistWithInvalidToken() throws Exception {
		when(client.authorizeTheRequest("wrongtoken")).thenReturn(false);
		this.mockMvc.perform(delete("/deleteSpecialist/{id}",201).header("Authorization", "wrongtoken"))
				.andExpect(status().isForbidden());
	}
	
	@Test
	@DisplayName("Test updatePackages() of the Controller without token")
	void testUpdatePackagesWithoutToken() throws Exception {
		PackageDetail packageDetail = new PackageDetail(101, "Package 3", Arrays.asList("OPT5", "OPT6"), 25000, 14);
		ObjectWriter ow = new ObjectMapper().writer().withDefaultPrettyPrinter();
		this.mockMvc.perform(put("/updatePackage/{packageId}",101).contentType(MediaType.APPLICATION_JSON)
				.content(ow.writeValueAsString(packageDetail))).andExpect(status().isBadRequest());
	}

	@Test
	@DisplayName("Test updatePackages() of the Controller with valid token")
	void testUpdatePackagesWithValidToken() throws Exception {
		when(client.authorizeTheRequest("@uthoriz@tionToken123")).thenReturn(true);
		PackageDetail packageDetail = new PackageDetail(101, "Package 3", Arrays.asList("OPT5", "OPT6"), 25000, 14);
		when(ipOfferingService.updateTreatmentPackage(101, packageDetail )).thenReturn(true);
		ObjectWriter ow = new ObjectMapper().writer().withDefaultPrettyPrinter();
		System.out.println(this.mockMvc.perform(
				put("/updatePackage/{packageId}",101).contentType(MediaType.APPLICATION_JSON)
				.content(ow.writeValueAsString(packageDetail))
						.header("Authorization", "@uthoriz@tionToken123"))
						.andReturn());
	}

	@Test
	@DisplayName("Test updatePackages() of the Controller with invalid token")
	void tesUpdatePackagesWithInvalidToken() throws Exception {
		when(client.authorizeTheRequest("wrongtoken")).thenReturn(false);
		PackageDetail packageDetail = new PackageDetail(101, "Package 3", Arrays.asList("OPT5", "OPT6"), 25000, 14);
		ObjectWriter ow = new ObjectMapper().writer().withDefaultPrettyPrinter();
		this.mockMvc.perform(put("/updatePackage/{packageId}",101).contentType(MediaType.APPLICATION_JSON)
				.content(ow.writeValueAsString(packageDetail)).header("Authorization", "wrongtoken"))
				.andExpect(status().isForbidden());
	}
